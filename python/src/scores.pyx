...
Available functions:
    ...
    scores.entropy : Compute ENTROPY score
...
score_entropy_base = attr.score_entropy_base
...
def rank_histogram(ens,verif,histogram_only=True):
    """rank_histogram,[ranks] = rank_histogram(ens,verif,[histogram_only=True])
       ...
       Inputs
       ------
       ens [rank-2 double array] : ensemble simulation (nvar,nens)
...
def crps_score_partition(double[:,::1] ens not None, double[::1] verif not None, int[::1] partition not None):
    ...
    c_crps_score_partition(<int>ens.shape[0], <int>ens.shape[1], <int>npartition, &crps_[0], &reliability_[0], &resolution_[0], &ens[0,0], &verif[0], &partition[0])
    return crps, reliability, resolution
...
def rcrv(ens,verif,partition=False):
    ...
    if numpy.isscalar(partition):
        ...
        crps,reliability,resolution = rcrv_score_partition(ens, verif, partition)
...
def rcrv_score_partition(double[:,::1] ens not None, double[::1] verif not None, int[::1] partition not None):
    ...
    c_rcrv_score_partition(<int>ens.shape[0], <int>ens.shape[1], <int>npartition, &ens_bias_[0], &ens_spread_[0], &ens[0,0], &verif[0], &partition[0])
    return ens_bias, ens_spread
...
def optimality(ens,obs,obs_std,partition=False):
    ...
    if obstype == 'normal' or obstype == 'gaussian' :
       if numpy.isscalar(obssigma):
          obssigma_ = numpy.zeros((ens.shape[0]), dtype=numpy.double) + obssigma
       else:
          obssigma_ = obssigma
...
def optimality_score_global(double[:,::1] ens not None, double[::1] obs not None):
    ...
    c_optimality_score_global(<int>ens.shape[0], <int>ens.shape[1], &ens_optimality, &ens[0,0], &obs[0])
...
def optimality_score_global_gaussian(double[:,::1] ens not None, double[::1] obs not None, double[::1] std_obs not None):
    ...
    c_optimality_score_global_gaussian(<int>ens.shape[0], <int>ens.shape[1], &ens_optimality, &ens[0,0], &obs[0], &std_obs[0])
    return ens_optimality
...
def optimality_score_partition(double[:,::1] ens not None, double[::1] obs not None, int[::1] partition not None):
    ...
    c_optimality_score_partition(<int>ens.shape[0], <int>ens.shape[1], <int>npartition, &ens_optimality_[0], &ens[0,0], &obs[0], &partition[0])
    return ens_optimality
...
def optimality_score_partition_gaussian(double[:,::1] ens not None, double[::1] obs not None, double[::1] std_obs not None, int[::1] partition not None):
    ...
    c_optimality_score_partition_gaussian(<int>ens.shape[0], <int>ens.shape[1], <int>npartition, &ens_optimality_[0], &ens[0,0], &obs[0], &std_obs[0], &partition[0])
    return ens_optimality
...
def entropy(ens,pref,events_outcome,score_only=True):
    ...
    """
    # Update Fortran module public variables
    attr.score_entropy_base = score_entropy_base
    # Associate callback routines in C-callable wrapper
...
def events_score(double[:,::1] ens not None, double[:,::1] pref not None):
    ...
    c_events_score(<int>ens.shape[0], <int>ens.shape[1], <int>pref.shape[0], <int>pref.shape[1], &score_[0], &ens[0,0], &pref[0,0])
    return score
...
def events_relative_entropy(double[:,::1] ens not None, double[:,::1] pref not None):
    ...
    c_events_relative_entropy(<int>ens.shape[0], <int>ens.shape[1], <int>pref.shape[0], <int>pref.shape[1], &relative_entropy_[0], &ens[0,0], &pref[0,0])
...
def events_cross_entropy(double[:,::1] ens not None, double[:,::1] pref not None):
    ...
    c_events_cross_entropy(<int>ens.shape[0], <int>ens.shape[1], <int>pref.shape[0], <int>pref.shape[1], &cross_entropy_[0], &entropy_[0], &ens[0,0], &pref[0,0])
...
def events_entropy(double[:,::1] ens not None, nevents not None, noutcomes not None):
    ...
    c_events_entropy(<int>ens.shape[0], <int>ens.shape[1], <int>nevents, <int>noutcomes, &entropy_[0], &ens[0,0])
    return entropy
...
def events_probability(double[:,::1] ens not None, nevents not None, noutcomes not None):
    ...
    c_events_probability(<int>ens.shape[0], <int>ens.shape[1], <int>nevents, <int>noutcomes, &pens_[0,0], &ens[0,0])
    return pens
