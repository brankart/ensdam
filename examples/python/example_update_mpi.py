...
if mpirank == 0:
  print('  First member of multiscale ensemble stored in file: update_example.nc')
  nc_ens = Dataset('update_example.nc', 'w', format='NETCDF4')
...
if mpirank == 0:
  print('-----------------------------------------------------------------')
  print('3. Generate members of the updated ensemble with the MCMC sampler')
  print('-----------------------------------------------------------------')

# Define cost function, with condition to constrain the updated ensemble
def cost(member):
  # Penalize value of ensemble members along the equator
  cost = np.sum( member[np.abs(lat1d)<1]**2 ,  dtype=np.double )
  cost = cost / 0.01
  # Sum contributions from every processor
  globcost = mpicomm.allreduce(cost, op=MPI.SUM)
  return globcost

# Generate updated ensemble
nup = 1 # number of updated members requested
maxchain = 5000 # number of iteration in the MCMC sampler
multiplicity = np.array([1, 4],np.intc) # mutliplicity of each scale in the Schur products
edam.update.control_print = 100
upens = edam.update.sample_mcmc(ensmulti,multiplicity,nup,maxchain,cost)

# Reconstruct global member from the contribution of every processor
mem1dglo = np.zeros_like(lon1dglo)
mem1dglo[mpirank::mpisize] = upens[0,:]
mpicomm.Allreduce(MPI.IN_PLACE, mem1dglo, op=MPI.SUM)

# Output updated member in NetCDF file
# Looking at the new member, we can see that the member looks like prior member
# with the condition provided by the cost function (small along the equator)
if mpirank == 0:
  print('  First member of updated ensemble stored in file: update_example.nc')
  nc_ens_up = nc_ens.createVariable('up', upens.dtype, ('lat','lon'))
  mem2d = np.reshape(mem1dglo, lon2d.shape)
  nc_ens_up[:,:] = mem2d
